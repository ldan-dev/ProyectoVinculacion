...
# -*- coding: utf-8 -*-
"""
Created on Wed Aug 13 12:18:36 2025
...
def capture_point_cloud():
    ...
    try:
        ...
        for i in range(n_frames):
            ...
            # Remove invalid points (too far or too close)
            o3d_pcd = o3d_pcd.select_by_index(np.where(np.asarray(vertices)[:, 2] > n_max)[0])
        ...
        #    xyz_filtered = xyz[(xyz[:, 2] > 2) & (xyz[:, 2] < 5.5)]  # Filtrar en el eje Z (distancia)

        # point_clouds = point_clouds[(point_clouds[:, 2] > n_min) & (point_clouds[:, 2] < n_max)]  # Filtrar en el eje Z (distancia)

#        point_clouds[:, 2] = point_clouds[]
